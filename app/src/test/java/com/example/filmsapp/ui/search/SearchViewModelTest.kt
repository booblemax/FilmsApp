...
class SearchViewModelTest {
    ...
    fun setUp() {
        ...
        coroutinesDispatcher.testCoroutineDispatcher.runBlockingTest {
            Mockito.`when`(repository.searchFilms(query1, 1)).thenReturn(
                Resource.SUCCESS(
                    populars.take(2).map { it.toModel() }
                )
            )
            Mockito.`when`(repository.searchFilms(query1, 2)).thenReturn(
                Resource.SUCCESS(
                    populars.subList(2, 4).map { it.toModel() }
                )
            )
            Mockito.`when`(repository.searchFilms(query2, 1)).thenReturn(
                Resource.SUCCESS(
                    upcoming.take(2).map { it.toModel() }
                )
            )
            Mockito.`when`(repository.searchFilms(query2, 2)).thenReturn(
                Resource.SUCCESS(
    ...
    //
    //     coroutinesDispatcher.testCoroutineDispatcher.runBlockingTest {
    //         Mockito.verify(repository, times(1)).searchFilms(query1, 1)
    //     }
    ...
    //     val origin: Resource<List<FilmModel>> = Resource.SUCCESS(populars.subList(2, 4).map { it.toModel() })
    //
    //     viewModel.loadFilms(query1)
    //     viewModel.loadFilms(query1)
    //     val result = viewModel.films.getOrAwaitValue()
    ...
    //     coroutinesDispatcher.testCoroutineDispatcher.runBlockingTest {
    //         Mockito.verify(repository, times(1)).searchFilms(query1, 1)
    //         Mockito.verify(repository, times(1)).searchFilms(query1, 2)
    //     }
    //     assertThat(origin, IsEqual(result))
    // }
